...
class Downloader {
    fun download(id: String, dir: File) {
        ...
        val response = YoutubeDL.getInstance().execute(request, DownloadProgressCallback { progress, size, rate, etaInSeconds ->
            Log.i("TAG", "download: $progress $size $rate $etaInSeconds")
        })
    }
}
